...
namespace AUTO_SCHE
{
    public partial class EMAIL_JOBS_JHERP : Form
    {
        thomas_class ts_conn = new thomas_class();
        string v_sqlstr;
        string v_TO_addr;

        public EMAIL_JOBS_JHERP()
        {
            InitializeComponent();
        }
        //每10秒更新DataView
        ...
        {
            // TODO: 這行程式碼會將資料載入 'dataSet1.ODS_JH_EMAIL_JHERP' 資料表。您可以視需要進行移動或移除。
            this.oDS_JH_EMAIL_JHERPTableAdapter.Fill(this.dataSet1.ODS_JH_EMAIL_JHERP);
        ...
        private void Do_job()
        {
            ...
            try
            {
                conn = new OleDbConnection(AUTO_SCHE.Properties.Settings.Default.JHERPDB);
                conn.Open();
                ...
                while (dr.Read())
                {
                    try
                    {
                        ...
                        ts_conn.trans_oleDbjherp(v_sqlstr, "update");
                    ...
                    catch (Exception ex)
                    {
                        ...
                        myUI("[失敗]序號：" + dr[6].ToString() + "。無法寄送。" + v_TO_addr.ToString() + "。" + ex.ToString() + "-" + System.DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss") + " \n", rTB_message);
                    }
                }
                dr.Close();
                conn.Close();
